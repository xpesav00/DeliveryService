...
<c:if test="${pageContext.request.secure}">
 <table border="1" bgcolor="lightblue" style="width: 95%;">
 <tr><th colspan="2" align="left">SSL enabled</th></tr>
 <tr><td>javax.servlet.request.cipher_suite</td><td>${requestScope['javax.servlet.request.cipher_suite']}</td></tr>
 <tr><td>javax.servlet.request.key_size</td><td>${requestScope['javax.servlet.request.key_size']}</td></tr>
 <c:if test="${! empty requestScope['javax.servlet.request.X509Certificate']}">
 <tr><th align="left">X509Certificate</th><th align="left">issuer</th></tr>
 <c:forEach items="${requestScope['javax.servlet.request.X509Certificate']}" var="c">
   <tr>
    <td><c:out value="${c.subjectDN}"/></td>
    <td><c:out value="${c.issuerDN}"/></td>
   </tr>
   <tr><td colspan="2"><pre style="overflow:auto; width: 800px; height: 500px;">${c}</pre></td></tr>
 </c:forEach>
 </c:if>
</c:if>
 </table>
...
<hr />
 ...
 <c:forEach items="${applicationScope}" var="a" >
   <tr>
    <td><c:out value="${a.key}"/></td>
    <td><c:out value="${fn:substring(a.value,0,60)}"/></td>
   </tr>
 </c:forEach>
 </table>
